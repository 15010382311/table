 ...
 * @version : V1.0
 */
public class ClassNOExcelUtils {
    ...
    **/ 
    public static XSSFWorkbook exportMultilevelHeader(String sheetName, String[][] head, List<?> dataList, Class type, String[][] tableEndData, OutputStream out,String titles) {
        ...
        int mergerNum = 0; //合并数
        //添加表格标题
        sheet.addMergedRegion(new CellRangeAddress(0, 0, 0,type.getDeclaredFields().length -2));
	 ...
	 for (int i = 0; i < dataList.size(); i++) {
        ...
        if (tableEndData != null && tableEndData.length > 0) {
            for (int i = 0; i < tableEndData.length; i++) {
                ...
                sheet.addMergedRegion(new CellRangeAddress(dataList.size() + m + i + 1, dataList.size() + m + i + 1, 0,  type.getDeclaredFields().length - 2));
